...
- `apps/web` Vue 3 PWA. Modules in `src/modules/<name>/{domain,application,infrastructure,ui}` with a public `index.ts`, plus one optional second entry `offline.ts` — query keys and the offline mutation registration only, no UI — which the composition root imports so a static import from `app/` cannot drag every screen into the entry chunk. Routed screens are exported from the barrel as async components for the same reason. Two modules own no data and only compose siblings through their barrels: `dashboard` (the home read model, `buildDashboard` is the pure core) and `plan` (the Plan screen with the income / expenses / budgets segments). A data module never imports another data module's screens; put a composed screen in one of these or add a new screen module. Follow the `/vue-ddd-architecture` skill.
