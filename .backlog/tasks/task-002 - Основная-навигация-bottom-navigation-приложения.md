...
Владелец выбрал паттерн Moni: **плавающая пилюля с четырьмя разделами и центральным «+»**. Форма панели меняется — это не прежний прилегающий таб-бар, а скруглённая до 999px пилюля, отставшая от краёв экрана; в светлой теме она тёмная, в тёмной первичный акцент берёт синий. Порядок вкладок, который уже был записан ниже, совпал с выбранным паттерном и не меняется. Шит, который открывает «+», собирается не здесь: это общий компонент быстрого действия из TASK-011, тот же, на котором стоят формы. Токены, радиус и тень приходят из TASK-004 — без них панель будет собрана в старом языке.
...
- По центру заметная круглая кнопка (+), выступающая над панелью. Открывает шит быстрых действий (записать баланс / перевод / поступление).
...
## Решения
- Действие (+): переиспользуем шит быстрых действий из TASK-011; содержимое меню отдельной задачей не меняем.
