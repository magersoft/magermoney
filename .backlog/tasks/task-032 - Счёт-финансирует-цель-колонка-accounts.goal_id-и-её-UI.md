...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 Миграция добавляет accounts.goal_id (nullable, FK на goals, on delete set null) без потери данных
- [ ] #2 PATCH /accounts/{id} принимает goalId: привязывает, отвязывает по null, отвечает 404 на чужую цель
- [ ] #3 Попытка привязать счёт, который уже держит другая цель, отклоняется 409 account_already_linked; архивная цель отклоняется 409
- [ ] #4 Шит привязки на экране цели перечисляет только свободные и неархивные счета, а когда свободных нет — называет причину, а не говорит «нет счетов»
