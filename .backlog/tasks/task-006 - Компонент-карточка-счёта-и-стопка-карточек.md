...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 Карточка показывает название счёта, баланс через amount lockup и иконку валюты
- [x] #2 Цвет карточки выводится из валюты детерминированно, а не задаётся вручную на каждом экране
- [x] #3 Счёт в валюте, отличной от базовой, визуально отличим от счёта в базовой
- [x] #4 Есть раскладка-полоса (горизонтальный скролл) и раскладка-стопка внахлёст; обе — из одного компонента карточки
- [x] #5 В полосе последним элементом идёт плитка добавления счёта
- [x] #6 Карточка — ссылка на счёт: цель нажатия >=44px, фокус с клавиатуры виден, порядок табуляции в стопке совпадает с визуальным
- [x] #7 Текст на карточке проходит AA на её заливке в обеих темах
- [x] #8 Тесты покрывают обе раскладки, плитку добавления и признак неосновной валюты
...
<!-- SECTION:NOTES:BEGIN -->
Цвет карточки. Валюта выбирает только тон: светлота и цветность фиксированы темой (--mm-tint-l / --mm-tint-c), поэтому AA чернил на заливке доказывается для всего круга сразу, а не выборочно. Тон раздаётся золотым углом по позиции валюты в CURRENCY_TINT_ORDER — списке, который сеет supabase/migrations; список только дописывается, вставка в середину перекрасит всё после неё. Неизвестный код получает тон из хеша FNV-1a.

Цветность упирается в охват, а не во вкус: у белого конус sRGB узкий, и первая версия (L 0.93 / C 0.055) выходила за него на 205 тонах из 360 — браузер свёл бы далёкие валюты в один цвет. Итог: светлая 0.86 / 0.06, тёмная 0.36 / 0.055, и tokens-contrast.test.ts теперь держит и AA, и охват по всему кругу.

Стопка внахлёст сделана зарезервированным местом, а не смещениями: каждый элемент, кроме последнего, занимает только высоту peek, карточка выходит за него, следующая закрашивает сверху. Поэтому порядок DOM и есть визуальный порядок, и tabindex нигде не нужен. Карточка под фокусом или курсором поднимается z-index — иначе кольцо фокуса срезала бы соседка.
