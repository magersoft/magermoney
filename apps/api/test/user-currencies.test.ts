...
describe('disconnect', () => {
  it('removes a currency nothing uses', async () => {
    ...
    expect((await disconnectCurrency(repo)(USER, 'COP')).isOk()).toBe(true);
  ...
  it('refuses while accounts still hold it, and says where', async () => {
    ...
    const err = (await disconnectCurrency(repo)(USER, 'COP'))._unsafeUnwrapErr();
    expect(err.code).toBe('CURRENCY_IN_USE');
    expect(err.message).toMatch(/account/i);
  });

  it('refuses while the profile still points at it', async () => {
    const repo = repoWith(['USD', 'COP']);
    repo.usageByCode['COP'] = {
      accounts: 0,
      budgets: 0,
    ...
    expect((await disconnectCurrency(repo)(USER, 'COP'))._unsafeUnwrapErr().code).toBe(
      'CURRENCY_IN_USE',
    );
  });
  ...
  it('refuses to remove the last one: a list with no currency shows nothing', async () => {
    ...
    expect((await disconnectCurrency(repo)(USER, 'USD'))._unsafeUnwrapErr().code).toBe(
      'LAST_CURRENCY',
    );
  });
  ...
  it('reports a currency that was never connected as missing', async () => {
    ...
    expect((await disconnectCurrency(repo)(USER, 'COP'))._unsafeUnwrapErr().code).toBe('NOT_FOUND');
  });
});
