...
export class PgRateRepository implements RateRepository {
  ...
  async upsertMany(rows: Omit<RateRow, 'quote'>[]): Promise<number> {
    ...
    const res = await this.sql`
      ...
      on conflict (base, quote, date, source, coalesce(user_id, '00000000-0000-0000-0000-000000000000'::uuid)) do update set value = excluded.value, refreshed_at = now()`;
    return res.count;
  }
  async lastApiRefreshAt(): Promise<Date | null> {
    const [row] = await this.sql<{ at: Date | null }[]>`
      select max(refreshed_at) as at from rates where source = 'api'`;
    return row?.at ?? null;
  }
