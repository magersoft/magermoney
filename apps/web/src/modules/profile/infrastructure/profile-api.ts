...
export const profileApi = (client: ApiClient) => ({
  get: async (): Promise<ProfileDto> => parse(await client.fetch('/me', { method: 'GET' }), ProfileDtoSchema),
  update: async (input: UpdateProfileInput): Promise<ProfileDto> =>
    parse(await client.fetch('/me', { method: 'PATCH', body: JSON.stringify(input) }), ProfileDtoSchema),
});
