...
### Second pass (offline replay)

- **A sheet closes on a parked write.** `record()` and `create()` resolve as soon as the write is either sent or paused, and the sheet closes with a toast (`offline.saved`) saying it is kept on the device; a parked write also stops counting as pending. Why: a paused mutation never settles, so the sheets that awaited `mutateAsync` stayed open with a disabled button for as long as there was no network. Cost if wrong: the person is told a write is kept before the server has seen it, so a later server rejection (a stale date, a gone account) surfaces only as a refetch that disagrees.
- **Each module gets a second public entry, `offline.ts`.** It carries the query keys and the mutation registration, no UI; `app/` imports it instead of the barrel, and the eslint `no-restricted-imports` pattern allows exactly `@/modules/*/offline`. Why: the barrels re-export every screen, so a static import from the composition root decides where the pages land. Measured: the barrels' pages were already split (as opaque `index-*.js` chunks), but the split now survives a static import, and the routed screens are exported as async components so the shell's own static import (`QuickActions` needs `useAccounts` and the sheets) cannot undo it. Cost if wrong: two public entries per module instead of one, and a page that is wanted synchronously must be imported from its file.
- **A parked write refuses to travel under another account's token.** The persisted variables carry the owner's user id; the mutation function compares it with the signed-in user and throws `ForeignWriteError` (never retried, so the write is dropped without a request). The id is not part of the request body — the API knows the user from the token. Why: a tab can be closed, signed out and signed in as somebody else while the write waits. Cost if wrong: a write is silently dropped where it could have been sent, which is the safe direction.
- **The retry budget stays at 3.** Unlimited retries would bring back the hanging sheet during an API outage while online: the mutation would never settle and never pause. Cost if wrong: a connection that flaps more than three times loses the write to a normal error, with its own message.
- **A lost response can still duplicate a journal entry on replay.** If the request reaches the API and only the answer is lost, the retry writes a second Balance entry. Accepted for a manual journal: the duplicate is the newest manual entry on its account, which is exactly the one the app lets you delete. Idempotency keys are deferred to a later phase. Cost if wrong: the owner deletes a duplicate by hand.
- **The account screen keeps padded zeros.** `Decimal.toFixed(scale)` prints `1.50`, not `1.5`, which is what the list already does through `Money.round()`; the two screens agreeing matters more than the shorter string.
