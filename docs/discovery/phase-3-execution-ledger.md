...
- Task 24: budgets mirror the expenses fixes even where the brief's code lacks them — `useBudgets` exposes `isError`/`refetch` and the segment renders `RouteError` inside `data-testid="budgets-error"` before the empty state; `summariseBudgets` lists as current everything with `activeTo === null || activeTo >= today`, so a budget starting next month is in the plan, not lost between the two lists (the brief's `isActiveOn` dropped it from both); the form has a not-found state with a back link; and Save is held until the limit is a Decimal above zero. Cost if wrong: none; each is the reading the Income and Expenses segments beside it already give.
- Task 24: the budget form uses a native `<select>` for currency, not the brief's shadcn `Select`/`SelectValue`, for the reason Task 23 recorded — reka-ui's `SelectValue` renders nothing for a value chosen before its content mounts, so the preset "EUR" would show as an empty trigger, and the e2e drives a native select with `selectOption`. Cost if wrong: three forms to convert together the day the project moves to the shadcn Select.
...
- Task 24's `budgets-api.test.ts` mock answers `init?.method ? dto : [dto]`, but `budgetsApi.list()` sends `{ method: 'GET' }`, so the list call received a single object and `listOf` refused it with "Expected an array" — the test could not pass against the brief's own (correct) implementation. Fixed in the test: `init?.method === 'GET' ? [dto] : dto`.
