...
- `ui`: `AccountsPage` (route `/`): amount lockup "Всего денег" (animated number change on display-currency switch), "Доступно до зарплаты", provider groups (provider icon/flag, name, group total in display currency), account rows (`CurrencyIcon`, name, kind label, balance in own currency large, display currency small), an "Archived" disclosure at the bottom, an "unconvertible" hint when any Account lacks a rate. `AccountDetailPage` (`/accounts/:id`): details, `note`, balance timeline (date, amount, delta to previous), actions "Record balance", "Transfer", menu "Edit / Archive / Delete" (Delete disabled with explanation when Transfers exist). `AccountFormPage` (a routed page at `/accounts/new` and `/accounts/:id/edit` rather than a sheet: the form is too long for a bottom sheet on a phone, and the back button closes it; create/edit; card fields appear when `kind = card`; currency locked when history exists). `RecordBalanceSheet` (large `MoneyInput`, date default now, note; reused for editing the latest manual entry).
